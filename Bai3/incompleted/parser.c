...
void compileAssignSt(void) {
  ...
  else if (lookAhead->tokenType == SB_ASSIGN) {
	  eat(SB_ASSIGN);
	  compileExpression();
  }
  else error(ERR_INVALIDASSIGN, lookAhead->lineNo, lookAhead->colNo);
...
void compileFactor(void) {
	switch (lookAhead->tokenType)
	{
	case TK_NUMBER: case TK_CHAR:
		compileUnsignedConst();
		break;
	case TK_IDENT:
		eat(TK_IDENT);
		if (lookAhead->tokenType == SB_LSEL)
